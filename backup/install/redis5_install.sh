...
#                       /____/            /____/
#
# 멀티 쉘 실행 : bash <(curl -f -L -sS http://shell.pe.kr/document/install/redis5_install.sh)
...
# -------------------------------------------------------------------------------------------------------------------
# Friendly Logo
printf "\e[00;32m|     ____           ___      \e[00m\n"
printf "\e[00;32m|    / __ \___  ____/ (_)____ \e[00m\n"
printf "\e[00;32m|   / /_/ / _ \/ __  / / ___/ \e[00m\n"
printf "\e[00;32m|  / _, _/  __/ /_/ / (__  )  \e[00m\n"
printf "\e[00;32m| /_/ |_|\___/\__,_/_/____/   \e[00m\n"
printf "\e[00;32m| :: Version ::    (v${REDIS_VERSION})   \n"
...
read -e -p ' (default. redis)> ' REDIS_HOME
...
# ----------------------------------------------------------------------------------------------------------------------
# 설치 여부 확인
if [[ -d "${SERVER_HOME}/${REDIS_HOME}" ]]; then
    printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
    printf "\e[00;32m|\e[00m \e[00;31m기존에 생성된 디렉토리가 있습니다. 삭제하고 다시 생성하려면 \"Y\"를 입력하세요.\e[00m\n"
    printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
    printf "\e[00;32m| Enter whether to install \"${REDIS_HOME}\" service\e[00m"
    ...
    if [[ "$(uppercase ${CHECK})" != "Y" ]]; then
        printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
        printf "\e[00;32m|\e[00m \e[00;31m\"${REDIS_HOME}\" 서비스 생성 취소...\e[00m\n"
        printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
        exit 1
    fi
fi
...
if [[ -z ${REDIS_PORT} ]]; then
    while [[ true ]]; do
        ...
        read -e -p " (default. 6379) > " REDIS_PORT
        if [[ -z ${REDIS_PORT} ]]; then
            REDIS_PORT=6379
            break
        ...
        #    REDIS_PORT=6379
        #    break
        elif [[ "$REDIS_PORT" -lt 1000 ]] || [[ "$REDIS_PORT" -ge 10000 ]]; then
            printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
            printf "\e[00;32m|\e[00m \e[00;31m포트 번호는 숫자로\e[00m \e[00;31m\"1000 ~ 10000\"\e[00m \e[00;32m까지만 입력 가능...\e[00m\n"
            printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
        else
            break
        fi
...
REDIS_PORT=`echo ${REDIS_PORT} | tr -d ' '`
...
## Redis Max Memory 설정.
printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
printf "\e[00;32m| 1k  : 1000 bytes\n"
printf "\e[00;32m| 1kb : 1024 bytes\n"
printf "\e[00;32m| 1m  : 1000000 bytes\n"
printf "\e[00;32m| 1mb : 1024*1024 bytes\n"
printf "\e[00;32m| 1g  : 1000000000 bytes\n"
printf "\e[00;32m| 1gb : 1024*1024*1024 bytes\n"
printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
printf "\e[00;32m| Enter the redis max memory\e[00m"
read -e -p ' (default. 256mb)> ' MAX_MEMORY
...
# ----------------------------------------------------------------------------------------------------------------------
printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
printf "\e[00;32m| REDIS_HOME  :\e[00m ${SERVER_HOME}/${REDIS_HOME}\n"
printf "\e[00;32m| REDIS_PORT  :\e[00m ${REDIS_PORT}\n"

# 서버 디렉토리 생성
mkdir -p ${SERVER_HOME}/${REDIS_HOME}/bin
mkdir -p ${SERVER_HOME}/${REDIS_HOME}/conf
mkdir -p ${SERVER_HOME}/${REDIS_HOME}/data
mkdir -p ${SERVER_HOME}/${REDIS_HOME}/logs
mkdir -p ${SERVER_HOME}/${REDIS_HOME}/work

## 바이너리 복사
cp ${REDIS_SRC_HOME}/src/redis-benchmark  ${SERVER_HOME}/${REDIS_HOME}/bin/
cp ${REDIS_SRC_HOME}/src/redis-check-aof  ${SERVER_HOME}/${REDIS_HOME}/bin/
cp ${REDIS_SRC_HOME}/src/redis-check-rdb  ${SERVER_HOME}/${REDIS_HOME}/bin/
cp ${REDIS_SRC_HOME}/src/redis-cli        ${SERVER_HOME}/${REDIS_HOME}/bin/
cp ${REDIS_SRC_HOME}/src/redis-sentinel   ${SERVER_HOME}/${REDIS_HOME}/bin/
cp ${REDIS_SRC_HOME}/src/redis-server     ${SERVER_HOME}/${REDIS_HOME}/bin/
cp ${REDIS_SRC_HOME}/src/redis-trib.rb    ${SERVER_HOME}/${REDIS_HOME}/bin/
...
# config:      ${SERVER_HOME}/${REDIS_HOME}/conf/redis.conf
# pidfile:     ${SERVER_HOME}/${REDIS_HOME}/work/redis.pid
#
...
# ---------------------------------------------------------------------------------
# Redis user is the default user of redis
export REDIS_USER='$USERNAME'

# ---------------------------------------------------------------------------------
# Redis home is the location of the configuration files of this instance of redis
export REDIS_HOME=\`cd \"\$PRGDIR/..\" >/dev/null; pwd\`

# Redis server
export REDIS_SERVER=\"\${REDIS_HOME}/bin/redis-server\"

# Redis pid
export REDIS_PID=\"\${REDIS_HOME}/work/redis.pid\"

# Redis configuration file
export REDIS_CONF_FILE=\"\${REDIS_HOME}/conf/redis.conf\"

# Redis log file
export REDIS_LOG=\"\${REDIS_HOME}/logs/redis.log\"

# 기본 디렉토리가 없는 경우 생성
if [[ ! -d \"\${REDIS_HOME}/logs\" ]]; then
    mkdir -p \${REDIS_HOME}/logs
fi
if [[ ! -d \"\${REDIS_HOME}/work\" ]]; then
...
logo() {
    ...
    echo
    printf \"Using REDIS_HOME: \${REDIS_HOME}\\\\n\"
    printf \"Using REDIS_PORT: ${REDIS_PORT}\\\\n\"
    echo
}
...
# Help
usage() {
    echo \"Script start and stop a Transcoder web instance by invoking the standard \${REDIS_HOME}/bin/redis.sh file.\"
...
# ---------------------------------------------------------------------------------
server_pid() {
    # pgrep -f \"\${REDIS_SERVER}\"
    echo \`ps aux | grep -v grep | grep \"\${REDIS_SERVER}\" | grep ${REDIS_PORT} | awk '{ print \$2 }'\`
}
...
start() {
    ...
    if [[ \"\${USER}\" = \"root\" ]]; then
        printf \"Redis 시작 중:\"
        su - \${REDIS_USER} -c \"\${REDIS_SERVER} \$REDIS_CONF_FILE\"
    elif [[ \"\${USER}\" = \"\${REDIS_USER}\" ]]; then
        printf \"Redis 시작 중:\"
        \${REDIS_SERVER} \$REDIS_CONF_FILE
    else
        printf \"You can not start this redis with \e[00;31m\${USER}.\e[00m\\n\"
        printf \"Do start with \e[00;32m\${REDIS_USER}\e[00m.\\n\"
        return 1
    fi
...
stop() {
    ...
    if [[ -n \"\$pid\" ]]; then
        # Stop daemons.
        if [ \"\${USER}\" != \"root\" ] && [ \"\${USER}\" != \"\${REDIS_USER}\" ]; then
            printf \"\e[00;31mYou can not start this redis with \${USER}.\e[00m\\n\"
            printf \"\e[00;31mDo stop with \${REDIS_USER}.\e[00m\\n\"
            return 1
        fi
...
exit \$retval
" > ${SERVER_HOME}/${REDIS_HOME}/bin/redis.sh
...
# REDIS_HOME is the location of the configuration files of this instance of redis
export REDIS_HOME=\`cd \"\$PRGDIR/..\" >/dev/null; pwd\`

\${REDIS_HOME}/bin/redis.sh start
" > ${SERVER_HOME}/${REDIS_HOME}/bin/start.sh
...
\${REDIS_HOME}/bin/redis.sh stop
" > ${SERVER_HOME}/${REDIS_HOME}/bin/stop.sh
...
\${REDIS_HOME}/bin/redis.sh restart
" > ${SERVER_HOME}/${REDIS_HOME}/bin/restart.sh
...
\${REDIS_HOME}/bin/redis.sh status
" > ${SERVER_HOME}/${REDIS_HOME}/bin/status.sh
...
\${REDIS_HOME}/bin/redis.sh log
" > ${SERVER_HOME}/${REDIS_HOME}/bin/log.sh

# -------------------------------------------------------------------------------------------------------------------
# redis.conf
echo "# Redis configuration file example.
#
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" > ${SERVER_HOME}/${REDIS_HOME}/conf/redis.conf
...
echo "# Protected mode is a layer of security protection, in order to avoid that
...
logfile \"${SERVER_HOME}/${REDIS_HOME}/logs/redis.log\"
...
dir ${SERVER_HOME}/${REDIS_HOME}/data/
...
if [[ -z ${REDIS_PASSWORD} ]]; then
    echo "# masterauth <master-password>
" >> ${SERVER_HOME}/${REDIS_HOME}/conf/redis.conf
else
...
echo "# When a replica loses its connection with the master, or when the replication
...
# crontab 에 등록
# 0 0 * * * ${SERVER_HOME}/${REDIS_HOME}/bin/log_rotate.sh

# 스크립트에 에러가 있는 경우 즉시 종료.
set -e

# 파일 경로와 파일명 분리.
FILE_PATH='${SERVER_HOME}/${REDIS_HOME}/logs'
FILE_EXTENSION='.log'
...
BACKUP_PATH='${SERVER_HOME}/${REDIS_HOME}/logs/archive'
...
" > ${SERVER_HOME}/${REDIS_HOME}/bin/log_rotate.sh

# 실행권한 할당.
chmod +x ${SERVER_HOME}/${REDIS_HOME}/bin/*.sh


# ----------------------------------------------------------------------------------------------------------------------
printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
printf "\e[00;32m| ${REDIS_HOME} create success...\e[00m\n"
printf "\e[00;32m|---------------------------------------------------------------------------------\e[00m\n"
