...
# 멀티 쉘 실행 : bash <(curl -f -L -sS http://shell.pe.kr/document/install/kpms_deploy_install.sh)
# Ubuntu에서 실행 : curl -f -L -sS  http://shell.pe.kr/document/install/kpms_deploy_install.sh -o /tmp/kpms_deploy_install.sh && chmod +x /tmp/kpms_deploy_install.sh && sudo /tmp/kpms_deploy_install.sh
...
if [[ "$(uname)" == "Linux" ]] && [[ "$USER" != "root" ]]; then
    printf "\e[00;31mKPMS Deploy 서버 설치 스크립트는 ROOT 권한으로 실행하십시오.\e[00m\n"
    exit 1
fi
...
KPMS_ALIAS="kpms-deploy"
KPMS_PROCESS_NAME="kpms.Deploy"
...
if [[ -d "${DEFAULT_SERVER_HOME}/${PROGRAME_HOME}/${KPMS_HOME}" ]]; then
    ...
    printf "\e[00;32m|\e[00m \e[00;31m기존에 설치된 KPMS Deploy Server가 있습니다. 삭제하고 다시 설치하려면 \"Y\"를 입력하세요.\e[00m\n"
    printf "\e[00;32m+---------------------------------------------------------------------------------\e[00m\n"
...
# ----------------------------------------------------------------------------------------------------------------------
if [[ "${OS}" == "darwin" ]]; then
    # Linux에서 필요하지 않은 파일 삭제.
    rm -rf ${SERVER_HOME}/${KPMS_ALIAS}/bin/linux
    rm -rf ${SERVER_HOME}/${KPMS_ALIAS}/bin/windows
    rm -rf ${SERVER_HOME}/${KPMS_ALIAS}/bin/*.bat
elif [[ "${OS}" == "linux" ]]; then
    # Linux에서 필요하지 않은 파일 삭제.
    rm -rf ${SERVER_HOME}/${KPMS_ALIAS}/bin/macos
    rm -rf ${SERVER_HOME}/${KPMS_ALIAS}/bin/windows
    rm -rf ${SERVER_HOME}/${KPMS_ALIAS}/bin/*.bat
...
" > /etc/systemd/system/${KPMS_ALIAS}.service
            ...
            # systemctl disable kpms-deploy && rm -rf /etc/systemd/system/kpms-deploy.service && systemctl reset-failed
            # systemctl list-unit-files | grep kpms
